...
public class SimulationSelfAvoidingRandomWalk_Exe15_34 extends Application {
    @Override
    public void start(Stage primaryStage) throws Exception {
        BorderPane pane = new BorderPane();
        WalkPane walkPane = new WalkPane();
        pane.setCenter(walkPane);

        Button btStart = new Button("Start");
        Button btReset = new Button("Reset");
        Button btAnimation = new Button("Animat");
        HBox hBox = new HBox(5);
        hBox.getChildren().addAll(btStart, btReset, btAnimation);
        pane.setBottom(hBox);
        hBox.setAlignment(Pos.CENTER);

        btStart.setOnAction(e -> walkPane.startWalk());
        btReset.setOnAction(e -> walkPane.resetWalk());
        btAnimation.setOnAction(e -> walkPane.startAnimation());

        primaryStage.setScene(new Scene(pane, 160, 200));
        primaryStage.setTitle("SimulationSelfAvoidingRandomWalk");
        primaryStage.show();
    }
    ...
    public class WalkPane extends Pane {
        ...
        private int cell = 16;
        private double widthX = 160;
        private double heightY = 160;
...
//                                + " = " + y);
                ...
                countStep += 2;
                countDedEnd = 0;
                System.out.println("countDedEnds = " + countDedEnds);
            }
